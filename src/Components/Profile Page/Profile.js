...
function Profile() {
    ...
    const nameEdit = async () => {
        await axios.patch('https://food-application-backend.herokuapp.com/api/auth/login');
    }
    const handleClick = async () => {
        try {
            console.log(user.user._id);
            const data = await axios.patch("https://food-application-backend.herokuapp.com/api/user/" + user.user._id,
