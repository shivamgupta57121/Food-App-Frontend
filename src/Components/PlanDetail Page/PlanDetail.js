...
function PlanDetail() {
    ...
    const { user } = useAuth();
    useEffect(async () => {
        const data = await axios.get(`https://food-application-backend.herokuapp.com/api/plan/${id}`) 
        console.log(data.data.data);
        delete data.data.data["_id"]
        delete data.data.data["__v"]
        setplan(data.data.data)
        const reviews = await axios.get("https://food-application-backend.herokuapp.com/api/getReview/" + id); // pending
    ...
    const handleClick = async () => {
        console.log(123645);
        const data = await axios.post("https://food-application-backend.herokuapp.com/api/review", {
            "review": review,
            "rating": rate,
            "user": user.user._id,
            "plan": id
    ...
    const handleDelete = async() =>{
        try{
            let data = await axios.delete("/", { // pending
                "id": id
            });
            alert(data);
        }
